...
with open(infilename, 'r') as f:
    ...
    for row in f.readlines():
        ...
        for i in range(sum_lb,sum_ub):
            likelihoods[i] = np.sum([odds[a][i] for a in row])

        log_likelihoods = np.zeros(sum_ub)
        for j in range(sum_lb,sum_ub):
            log_likelihoods[j] = np.sum([np.log(odds[b][j]) for b in row])

        # Bayes rule: the posterior is proportional to the likelihood times the prior
        unnormalized_posteriors = np.exp(np.add(log_likelihoods,np.log(priors)) - np.max(np.add(log_likelihoods,np.log(priors))))
        # show_array("Unnormalized posteriors", unnormalized_posteriors)

        # But they don't sum to 1.0, so we need scale them so they do
        marginal_likelihood = np.sum(unnormalized_posteriors)#np.sum(np.multiply(likelihoods,priors))
